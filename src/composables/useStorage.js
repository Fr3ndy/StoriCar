...
// ── Mapper DB (snake_case) → JS (camelCase) ──────────────────
...
async function loadData() {
  ...
  loadingPromise = (async () => {
    try {
      console.log('[useStorage] loadData - inizio fetch Supabase')
      ...
      console.log('[useStorage] loadData - fetch completato, vehicles:', vehicles?.length, 'fuel:', fuelRecords?.length)
    } catch (e) {
      console.error('[useStorage] loadData error:', e)
    } finally {
      console.log('[useStorage] loadData - finally, dataReady = true')
...
// Carica subito se c'è già una sessione attiva
...
supabase.auth.onAuthStateChange(async (_event, session) => {
  ...
  // INITIAL_SESSION: token già pronto, carica subito se non ancora fatto
  if (_event === 'INITIAL_SESSION' && session?.user && !dataReady.value) {
    console.log('[useStorage] INITIAL_SESSION - chiamo loadData')
    loadingPromise = null
    await loadData()
  }
})
...
export function useStorage() {
  ...
  function getVehicle(id) {
    return data.value.vehicles.find(v => v.id === id)
  }
  ...
  function getFuelRecord(id) {
    return data.value.fuelRecords.find(r => r.id === id)
  }
  ...
  function getLastFuelRecord(vehicleId) {
  ...
  function getExpense(id) {
    return data.value.expenses.find(e => e.id === id)
  }
  ...
  function getDeadline(id) {
    return data.value.deadlines.find(d => d.id === id)
  }
  ...
  function getRecurringPayment(id) {
    return data.value.recurringPayments.find(p => p.id === id)
  }

  function getRecurringPaymentsByVehicle(vehicleId) {
    return data.value.recurringPayments.filter(p => p.vehicleId === vehicleId)
  }
